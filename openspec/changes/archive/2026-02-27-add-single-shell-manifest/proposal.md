...
## What Changes
- Add a single shell-manifest data model under `home/.chezmoidata/shell_manifest/` that declares aliases, exports, functions, and init snippets.
...
- Co-locate package install metadata in per-domain manifests using `package_catalog` blocks.

## Impact
- Affected specs: `shell-manifest` (new capability)
- Affected code (implementation stage): `home/.chezmoidata/shell_manifest/`, `home/.chezmoitemplates/aliases.tmpl`, `home/.chezmoitemplates/exports.tmpl`, `home/.chezmoitemplates/functions.tmpl`, renderer helper templates, and tests
- User-visible outcome: shell behavior can be added or removed from one manifest model while preserving deterministic output
