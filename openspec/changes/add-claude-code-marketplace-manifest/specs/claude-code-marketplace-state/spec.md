...
Claude Code marketplace state SHALL be declared in manifest TOML files under a dedicated chezmoi data directory, with each file representing exactly one marketplace and the file name matching the marketplace name.

#### Scenario: Add marketplace by adding one file

- **WHEN** a new managed Claude Code marketplace is added
- **THEN** the marketplace SHALL be declared in a new TOML file named after that marketplace
- **AND** the file SHALL contain the marketplace source and profile metadata for only that marketplace
